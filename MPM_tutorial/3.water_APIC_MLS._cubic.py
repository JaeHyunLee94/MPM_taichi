...
def substep():
    ...
    for p in ti_particle_pos:
        ...
        w = [(1 / 6) * (2 - ti.abs(fx)) ** 3, 0.5 * (ti.abs(fx - 1)) ** 3 - (fx - 1) ** 2 + 2 / 3,
             0.5 * (ti.abs(2 - fx)) ** 3 - (2 - fx) ** 2 + 2 / 3, (1 / 6) * (2 - ti.abs(fx - 3)) ** 3]
        ...
        stress = dt * 3 * (bulk_modulus * ((1 / ti_particle_Jp[p]) ** gamma - 1) * ti_particle_Jp[
    ...
    for p in ti_particle_pos:
        ...
        for i, j, k in ti.static(ti.ndrange(4, 4, 4)):
            ...
            new_C += 3 * weight * ti_grid_vel[base + offset].outer_product(dpos) / grid_dx ** 2
