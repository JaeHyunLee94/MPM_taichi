...
grid_res = 64
particle_num = 8 * (grid_res ** 3)  ##python global variable : not updated in taichi kernel
particle_rho = 1
...
ti_grid_vel_old = ti.Vector.field(3, ti.f32, shape=(grid_res, grid_res, grid_res))
...
def substep():
    ...
    for p in ti_particle_pos:
        ...
        for i, j, k in ti.static(ti.ndrange(3, 3, 3)):
            ...
            ti_grid_vel_old[base + offset] += weight * (particle_mass * ti_particle_vel[p])
    ...
    for i, j, k in ti_grid_mass:
        ...
        if ti_grid_mass[i, j, k] > 0:
            ...
            ti_grid_vel_old[i, j, k] /= ti_grid_mass[i, j, k]
    ...
    for p in ti_particle_pos:
        ...
        old_v = ti.zero(ti_particle_vel[p])
        ...
        for i, j, k in ti.static(ti.ndrange(3, 3, 3)):
            ...
            old_v += weight * ti_grid_vel_old[base + offset]
            new_v_grad += ti_grid_vel[base + offset].outer_product(d_weight)

        # particle update
        ti_particle_vel[p] = new_v + alpha * (ti_particle_vel[p] - old_v)
